...
{
    public class TcpRemoteGateClient : IRemoteGateClient
    {
        ...
        /* Locking */
        private String m_SyncRoot = "TcpRemoteGateClient Sync";
        ...
        private XPortCommBody ContactServerWithParams(String operation, String[] parameters, String content)
        {
            ...
            sb.Append(Uri.EscapeDataString(operation));
            ...
            Byte[] b_Request = Encoding.UTF8.GetBytes(sb.ToString());

            try
            {
                TcpRequest request = new TcpRequest(m_Host, m_PortNumber);
                Byte[] bytes = request.GetResponse(b_Request);

                String respString = Encoding.UTF8.GetString(bytes);
        ...
        {
            return ContactServerWithParams(operation, new String[] { clientId }, null);
        }
        ...
        {
            return ContactServerWithParams(operation, new String[] { clientId, scriptId }, null);
        }
        ...
        {
            return ContactServerWithParams(operation, new String[] { clientId }, new ScriptSerializer(script).Serialize());
        } 
        ...
        private String Operation_REGISTER(String securityKey)
        {
            ...
            if (securityKey != null)
                response = ContactServerWithParams("REGISTER", new String[] { securityKey }, null);
            else
        ...
        private List<ScalarSet[]> SplitBySize(List<ScalarSet> sets)
        {
            List<ScalarSet[]> ss = new List<ScalarSet[]>();
            ss.Add(sets.ToArray());
            
            return ss;
            /*
            Int32 MaxBytesPerRequest = 1024 * 64;
            StringBuilder sb = new StringBuilder();
            List<ScalarSet> currentSet = new List<ScalarSet>();
            List<ScalarSet[]> setList = new List<ScalarSet[]>();

            for (Int32 i = 0; i < sets.Count; i++)
            {
                DataSerializer.Serialize(sb, sets[i]);
                currentSet.Add(sets[i]);

                if (sb.Length >= MaxBytesPerRequest)
                {
                    setList.Add(currentSet.ToArray());
                    currentSet.Clear();
                    sb.Length = 0;
                }
            }
                ...
                while (true)
                {
                    ...
                    foreach (IScript script in scriptList)
                    {
                        ...
                        if (scriptId != null)
                        {
                            ...
                            {
                                List<ScalarSet[]> newSplit = SplitBySize(setList);

                                foreach (ScalarSet[] newSetList in newSplit)
                                {
                                    Boolean success = false;

                                    try
                                    {
                                        success = Operation_DATAIN(m_ClientId, scriptId, newSetList);
                                    }
                                    catch
                                    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="TcpRemoteGateClient"/> class.
        /// </summary>
        /// <param name="host">The host.</param>
        /// <param name="portNumber">The port number.</param>
        /// <param name="securityKey">The security key.</param>
        /// <param name="pollTimeMs">The poll time in milliseconds.</param>
        public TcpRemoteGateClient(String host, Int32 portNumber, String securityKey, Int32 pollTimeMs)
        {
            if (host == null)
        ...
        /// <param name="portNumber">The port number.</param>
        /// <param name="pollTimeMs">The poll time in milliseconds.</param>
        public TcpRemoteGateClient(String host, Int32 portNumber, Int32 pollTimeMs)
        {
            if (host == null)
                throw new ArgumentNullException("host");
