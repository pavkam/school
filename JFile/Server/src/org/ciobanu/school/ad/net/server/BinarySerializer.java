...
public class BinarySerializer implements ControlPacketSerializer
{
	...
	private byte[] serializePacket(ControlPacket packet) throws IOException
	{
		/* Build output */
	...
	private ControlPacket deserializePacket(byte[] packetBuff, int offset, int length) throws IOException
	{	
		ByteArrayInputStream bin = new ByteArrayInputStream(packetBuff, offset, length);
		DataInputStream din = new DataInputStream(bin);
		
		EditableControlPacket packet = new DataPacket();
	...
	/* Actual Networking */
	public ControlPacket receivePacket(int timeOutMillis) throws IOException
	{
		...
		while (true)
		{
			...
			if (lengthToExpect != null && (lengthToExpect + 4) <= bytePacket.size())
			{
				...
				ControlPacket cPacket = deserializePacket(allBuffer, 4, lengthToExpect);
	...
	public void sendPacket(ControlPacket packet) throws IOException
	{
